 ...
 * Step: CONTACT-V1-5
 * Version: DPRO-CONTACT-1-WORKER-20260824-GMAIL-SENT-COPY-R7-STAGED
 ...
 *   WEB_EMAIL_FROM_NAME=DPRO SHOP
 *   WEB_EMAIL_INBOUND_ENABLED=false
 *   WEB_EMAIL_FORWARD_TO=dpromstk2000@gmail.com
 *   WEB_EMAIL_SENT_COPY_ENABLED=false
 *   WEB_EMAIL_SENT_COPY_TO=optional (falls back to WEB_EMAIL_FORWARD_TO)
 *
 ...
 *   RESEND_API_KEY (when WEB_EMAIL_REPLY_ENABLED=true)
 *
...
const VERSION = "DPRO-CONTACT-1-WORKER-20260824-GMAIL-SENT-COPY-R7-STAGED";
...
function webChannelCode(env) {
  return value(env, "WEB_CHANNEL_CODE", "DPRO_SHOP_WEB");
}
...
function webEmailFromAddress(env) {
  return value(env, "WEB_EMAIL_FROM_ADDRESS", "reply@dpro-shop.com").toLowerCase();
}

function webEmailFromName(env) {
  return value(env, "WEB_EMAIL_FROM_NAME", "DPRO SHOP")
    .replace(/[\r\n]+/g, " ")
    .trim()
    .slice(0, 120) || "DPRO SHOP";
}
...
function validateWebEmailReplyEnv(env) {
  ...
  const fromAddress = normalizeWebEmail(webEmailFromAddress(env));
  if (!fromAddress.endsWith("@dpro-shop.com")) {
    const e = new Error("WEB_EMAIL_FROM_ADDRESS_must_use_dpro_shop_domain");
    e.status = 503;
    throw e;
  }
...
async function sendWebAttachmentBatch(
  ...
  try {
    const resendResult = await resendApi(
      env,
      {
...
function webEmailSubject() {
  return "【DPRO SHOP】お問い合わせへのご返信";
}

function webEmailBody(text) {
  return [
    text,
    "",
    "――――――――――",
...
async function handleWebEmailInbound(message, env) {
  const threadId = webReplyThreadIdFromRecipient(env, message?.to);
  if (!threadId) {
